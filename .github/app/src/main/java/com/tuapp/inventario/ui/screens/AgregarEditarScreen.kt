    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        if (esEdicion) "Editar repuesto" else "Agregar repuesto",
        ...
        Column(
            ...
            ExposedDropdownMenuBox(expanded = expandirTipo, onExpandedChange = { expandirTipo = it }) {
                ...
                ExposedDropdownMenu(expanded = expandirTipo, onDismissRequest = { expandirTipo = false }) {
                    TipoRepuesto.values().forEach { t ->
                        DropdownMenuItem(text = { Text(t.label) }, onClick = { tipo = t.label; expandirTipo = false })
                    }
                }
            }
            ...
            OutlinedTextField(
                value = marca, onValueChange = { marca = it; marcaError = false },
            ...
            OutlinedTextField(
                value = modelo, onValueChange = { modelo = it; modeloError = false },
            ...
            ExposedDropdownMenuBox(expanded = expandirEstado, onExpandedChange = { expandirEstado = it }) {
                ...
                ExposedDropdownMenu(expanded = expandirEstado, onDismissRequest = { expandirEstado = false }) {
                    EstadoRepuesto.values().forEach { e ->
                        DropdownMenuItem(text = { Text(e.label) }, onClick = { estado = e.label; expandirEstado = false })
                    }
                }
            }
            ...
            Button(
                ...
                Text(
                    if (esEdicion) "Guardar cambios" else "Agregar repuesto",
                    fontWeight = FontWeight.Bold
                )
            }
